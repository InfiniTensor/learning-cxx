...
int main(int argc, char **argv) {
    ...
    }
    {
        ...
        ASSERT(!ref, "Fill in `ref` or `!ref`");
        // THINK: WHAT and WHY?
        //若你想要的是“拷贝当前布尔值，不影响容器”，写 bool ref = vec[30];（这时修改 ref 不会影响 vec）。
        //若想显式表示代理类型，可写 auto ref = std::vector<bool>::reference(vec[30]); 
        //或 std::vector<bool>::reference ref = vec[30];（行为同当前代码，修改会影响容器）。
