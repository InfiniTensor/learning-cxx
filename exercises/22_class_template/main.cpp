...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        ...
        for (unsigned int idx = 0; idx < size; ++idx) {
            ...
            // 根据广播规则计算others中对应元素的索引
            unsigned int other_idx = 0;
            unsigned int multiplier = 1;
            for (int d = 3; d >= 0; --d) {
                // 如果others该维度为1，则广播，索引为0
                unsigned int other_indices_d = (others.shape[d] == 1) ? 0 : indices[d];
                other_idx += other_indices_d * multiplier;
                multiplier *= others.shape[d];
            }
