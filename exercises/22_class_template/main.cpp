...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        // TODO: 实现单向广播的加法
        // 检查兼容性：others.shape[d] == shape[d] 或者 == 1
        for (int d = 0; d < 4; ++d) {
            ASSERT(others.shape[d] == shape[d] || others.shape[d] == 1, "incompatible shapes for broadcasting");
        }
            ...
            for (unsigned int i1 = 0; i1 < shape[1]; ++i1) {
                for (unsigned int i2 = 0; i2 < shape[2]; ++i2) {
                    for (unsigned int i3 = 0; i3 < shape[3]; ++i3) {
                        ...
                        unsigned int oidx = ((o0 * others.shape[1] + o1) * others.shape[2] + o2) * others.shape[3] + o3;
