...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        ...
        for (unsigned int n = 0; n < shape[0]; ++n) {
            unsigned int n_off = (n % others.shape[0]) * o_strides[0];

            for (unsigned int c = 0; c < shape[1]; ++c) {
                unsigned int c_off = (c % others.shape[1]) * o_strides[1];

                for (unsigned int h = 0; h < shape[2]; ++h) {
                    unsigned int h_off = (h % others.shape[2]) * o_strides[2];
                    
                    for (unsigned int w = 0; w < shape[3]; ++w) {
                        unsigned int w_off = (w % others.shape[3]) * o_strides[3];
                        unsigned int other_idx = n_off + c_off + h_off + w_off;
